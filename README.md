...
- Copy text modal (toolbar macro tray `select` on mobile, header `copy` on desktop) — bottom-sheet on mobile, centered floating dialog on desktop, with a one-tap `Copy` (Clipboard API + `execCommand` + manual fallbacks), `Select all`, a `visible` / `full scrollback` scope toggle (the default `visible` snapshot is the current viewport plus a few screens of recent scrollback above it, so copying spans more than one screenful without dumping the entire 10k-line buffer), soft-wrap so long lines don't need horizontal scrolling, and a line/char meta line
