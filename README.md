...
- Browser-driven PTY resize sends now wait longer for layout to settle before forwarding width/height changes, which reduces extension/sidebar animation churn that can kick redraw-heavy TUIs like Claude Code into broken blank-line redraws
...
- Ghostty runtime bumped to `@andrewting19/ghostty-web@0.5.7` on this branch — it keeps the 0.5.6 parity fixes, pauses canvas paints during DEC synchronized output, renders the first echo response immediately outside synchronized output, and clears stale cells when scroll growth reuses row memory
- iPad Safari with a hardware keyboard is now stable under both xterm and Ghostty — focusing the terminal no longer reveals the browser accessory-bar/page-scroll bug, and two-finger keyboard-trackpad scrolling still works inside terminal scrollback
...
- `Shift+Enter` inserts one newline as bracketed-paste text in both terminal renderers. This works in Pi and zsh without submitting the prompt or printing Pi-specific key codes. The active application's paste mode is preserved by snapshot reconnect; plain `Enter`, `Alt+Enter`, text inputs outside the terminal, and IME composition are unchanged. If an application has not enabled bracketed paste (for example older bash), the terminal shows a notice and sends nothing instead of accidentally submitting. This is text insertion, not a custom Shift+Enter binding in the application. See the [xterm bracketed paste protocol](https://invisible-island.net/xterm/ctlseqs/ctlseqs.html#h3-Bracketed-Paste-Mode).
...
- Copy text modal (toolbar macro tray `select` on mobile, header `copy` on desktop) — bottom-sheet on mobile, centered floating dialog on desktop, with a one-tap `Copy` (Clipboard API + `execCommand` + manual fallbacks), `Select all`, a `visible` / `full scrollback` scope toggle (`visible` strictly matches the current viewport; `full scrollback` is computed lazily only when requested so opening stays cheap), the scroller opens pinned to the bottom on both scopes so the latest output is what you see first, hard-wrap on a `<pre>` to keep the 10k-line scrollback case responsive (soft-wrap on a `<textarea>` made the browser lay out every wrap point and stalled the modal), and a line/char meta line
...
- Browser-side reconnect comparison helper — the dev build now also exposes `window.__wt_attach_metrics.compareSession()` / `.compareAll()` so raw-vs-snapshot attach can be measured directly from a remote browser console (useful for iPad/Tailscale validation without shell access)
  - Convenience helpers: `__wt_attach_metrics.sessions()` lists live session IDs/names and `__wt_attach_metrics.compareCurrent()` compares the currently attached session without needing to copy its ID first
  - One-shot remote study helpers: `__wt_attach_metrics.studyCurrent()` and `.studyAll()` package the comparisons together with renderer, user agent, URL, and `/api/sidecar` status for easy copy/paste from a remote browser
  - Clipboard helpers: `__wt_attach_metrics.copyStudyCurrent()` and `.copyStudyAll()` also copy the JSON report to the clipboard, which is useful on iPad when you want to paste the result elsewhere without inspecting the console object tree
  - Header terminal helpers: the old dev-only `study` button has been replaced with always-available `copy` and `input` buttons. `copy` opens a native scrollback selection sheet, while `input` opens the same text/paste/dictation sheet used by the mobile toolbar.
...
- Live post-viewport study baseline (April 3, 2026) — after landing Ghostty viewport restoration, running `bun run scripts/study-live-sessions.ts both captures/study-both-2026-04-03-post-viewport` against the current dev server added a larger normal-buffer agent session:
  - `Cho'Gath` (normal): `320,636` raw bytes, `85,566` xterm snapshot bytes, `84,527` Ghostty rendered-text bytes; reconnect timings were xterm raw `~1.08ms` vs snapshot `~6.26ms`, Ghostty raw `~2.06ms` vs snapshot `~3.74ms`
  - `Tahm Kench` (alternate) remained an extreme win for snapshot reconnect at roughly `~2.0x10^3` raw-to-snapshot shrinkage and `~1.7-2.1ms` snapshot attach versus `~14-16ms` raw attach
  This reinforces the current product tradeoff: snapshot reconnect is the right correctness path and wins decisively on large alternate-screen sessions, while some normal-buffer sessions still favor raw on localhost latency even after payload reduction.
