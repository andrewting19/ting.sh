...
- Copy text modal (toolbar macro tray `select` on mobile, header `copy` on desktop) — bottom-sheet on mobile, centered floating dialog on desktop, with a one-tap `Copy` (Clipboard API + `execCommand` + manual fallbacks), `Select all`, a `visible` / `full scrollback` scope toggle (`visible` strictly matches the current viewport so the snapshot is exactly what's drawn on screen; `full scrollback` dumps the whole buffer), the textarea opens scrolled to the bottom on both scopes so the latest output is what you see first, soft-wrap so long lines don't need horizontal scrolling, and a line/char meta line
