...
- Ghostty runtime bumped to `@andrewting19/ghostty-web@0.5.5` on this branch — line-height is honored, OSC 52 clipboard writes propagate to the browser clipboard, macOS Option is treated as Meta, freed terminals no longer leak stale cells, long scrollback stays stable, mouse-tracked TUIs receive wheel coordinates correctly, FitAddon no longer reserves a fake scrollbar gutter that left a right-edge gap, and `OSC 10/11/12` color queries are answered in JS instead of spamming WASM warnings
