...
## Bugs (lower priority)
- [ ] macOS TCC can silently brick the detached dev `ptyd` — the nohup-orphaned sidecar inherits the spawning terminal app's (e.g. Ghostty's) file-access identity as its permanent TCC "responsible process"; when that app's grant goes stale (typically after the app auto-updates), the daemon gets `EPERM` on every `~/Documents` read, `/health` 500s, and `bun run dev` dies with a misleading `ptyd failed to start on port 7781` while the old daemon still holds the port. Quick remedy: toggle the terminal app's Files & Folders / Full Disk Access off+on in System Settings (heals the live daemon instantly, sessions preserved). Durable fix: bootstrap the dev sidecar as a launchd user agent (`launchctl bootstrap gui/$UID`) so it is its own TCC responsible process and a one-time grant to `bun` survives terminal-app updates
- [ ] Harden `ptyd` `/health` against fingerprint read failures — serve the cached boot-time fingerprint plus an explicit `fingerprintError` field instead of 500ing when `computeSidecarRuntimeInfo` throws (e.g. TCC `EPERM`), so a permissions hiccup degrades stale-detection instead of taking down dev startup and every `/ws` attach
- [ ] `dev-ensure-ptyd` should diagnose "unhealthy daemon still holding the port" — when `/health` fails but the port is occupied, report the daemon pid, tail the ptyd log for the underlying error, and suggest the TCC re-toggle remedy instead of the generic `ptyd failed to start` timeout
...
- [x] iPad hardware-keyboard focus/scroll regressions — fixed (both renderers now avoid Safari's accessory-bar/page-scroll bug without sacrificing in-terminal two-finger keyboard-trackpad scrolling)
...
- [x] Browser-side clipboard study helper — `__wt_attach_metrics.copyStudyCurrent()` / `.copyStudyAll()` now also copy the JSON report to the clipboard for easier iPad/Tailscale reporting
- [x] Header terminal helper buttons — desktop/tablet header now exposes `copy` for native scrollback selection and `input` for the shared paste/dictation sheet where the dev-only `study` button used to live
...
- [x] Post-viewport live study baseline recorded — after landing Ghostty viewport restore, a later live study added a larger normal-buffer `Cho'Gath` session (`320,636` raw bytes vs `85,566` xterm snapshot bytes vs `84,527` rendered-text bytes) showing that some real normal-buffer sessions still favor raw localhost latency even though snapshot remains the correct reconnect path
...
- [x] Ghostty normal-buffer viewport restore — rendered-text snapshots now carry the saved normal-buffer viewport line and Ghostty restore reapplies it after snapshot write, with browser coverage for scrolled-up reconnects
- [x] Ghostty snapshot reconnect parity — visible content, semantic scrollback, and saved viewport position are now preserved on reconnect; the remaining xterm-vs-Ghostty `baseY` difference is treated as an internal renderer-model mismatch rather than a user-visible correctness bug
