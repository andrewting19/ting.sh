...
## Bugs (lower priority)
- [x] macOS TCC can silently brick the detached dev `ptyd` — fixed (detached spawns now bootstrap through `launchctl` as a `sh.ting.ptyd.<port>` launchd user agent via `src/ptydSpawn.ts`, so the daemon is its own TCC responsible process and a one-time Documents grant to `bun` survives terminal-app updates; verified live — the grant now sits on `~/.bun/bin/bun` instead of the terminal app. Quick remedy if an old-style daemon breaks again: toggle its terminal app's file access off+on in System Settings, which heals it in place with sessions preserved)
- [x] Harden `ptyd` `/health` against fingerprint read failures — fixed (`tryComputeSidecarRuntimeInfo` catches read errors; `/health` stays `ok: true` with null fingerprints and a `fingerprintError` field instead of 500ing, so a TCC hiccup degrades stale-detection instead of taking down dev startup and every `/ws` attach)
- [x] `dev-ensure-ptyd` should diagnose "unhealthy daemon still holding the port" — fixed (probe now distinguishes down vs unhealthy; when a daemon holds the port with a failing `/health` it reports the pid, tails the ptyd log, and prints the TCC re-toggle remedy instead of blind-spawning into a bind failure and a misleading timeout)
