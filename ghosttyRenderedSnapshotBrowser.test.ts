...
test("ghostty rendered-text VT restore preserves box drawing glyphs", async () => {
  const source = new XtermVtSnapshotTracker(24, 8, 200);
  await source.write(Buffer.from("╭────╮\r\n│ menu │\r\n╰────╯\r\n", "utf8"));

  const renderedSnapshot = captureRenderedTextSnapshot(source.terminal);
  const renderedVt = renderedTextSnapshotToVt(renderedSnapshot);
  expect(renderedVt).toContain("╭");
  expect(renderedVt).toContain("│");
  expect(renderedVt).not.toContain("â");
...
<html>
    ...
    <script type="module" src="/app.js"></script>
      ...
      if (url.pathname === "/app.js") {
        return new Response(`
          ...
          term.open(document.getElementById("terminal"));
          await new Promise((resolve) => term.write(snapshot.payload, resolve));
          window.__LINES__ = snapshotBuffer(term.buffer.normal);
        `, {
          headers: { "content-type": "text/javascript; charset=utf-8" },
        });
      }
  ...
  try {
    const page = await browser.newPage();
    await page.goto(`http://127.0.0.1:${server.port}/`);
    await page.waitForFunction(() => Array.isArray((window as Window & { __LINES__?: unknown }).__LINES__));
    const lines = await page.evaluate(() => (window as Window & { __LINES__: string[] }).__LINES__);
