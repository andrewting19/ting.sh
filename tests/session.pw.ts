...
import { getSessions, loadWithRenderer, newSession, getTerminalText, waitForTerminal, waitForPrompt, switchToSession, killAllSessions } from './helpers'
...
test('url hash — switching session updates hash to host/sessionName', async ({ page }) => {
  ...
  const hostId = (await getHash(page)).split('/')[0]
  const expected1 = `${hostId}/${name1}`
